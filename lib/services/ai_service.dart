...
class QuillBotService {
  ...
  Future<void> initialize() async {
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageFinished: (String url) {
            ...
            unawaited(_enableLightMode());
          },
          onWebResourceError: (WebResourceError error) {
            debugPrint(
              '[QuillBotService] Web resource error: ${error.description}',
            );
          },
          onNavigationRequest: (NavigationRequest req) {
            if (req.url.contains('quillbot.com')) {
    ...
    unawaited(_controller.loadRequest(Uri.parse('https://quillbot.com/ai-chat')));
    _isInitialized = true;
  }
  ...
  Future<List<String>> fetchHelpfulTips() async {
    ...
    try {
      final String rawResponse = await _sendDirectApiRequest(prompt);
      final dynamic json = _cleanAndParseJson(rawResponse);

      if (json != null && json is List<dynamic>) {
        return json.map((dynamic e) => e.toString()).toList();
      } else if (json != null &&
          json is Map<String, dynamic> &&
          json.containsKey('tips')) {
        return (json['tips'] as List<dynamic>)
...
''';
  ...
  Future<String> chat(String userMessage, {String? systemContext}) async {
    ...
    final String prompt = systemContext == null || systemContext.isEmpty
        ? userMessage
        : '$systemContext\n\n$userMessage';
  ...
  Future<String> _sendDirectApiRequest(String text) async {
    ...
    final String safeText = jsonEncode(text);
  ...
  dynamic _cleanAndParseJson(String raw) {
    try {
      ...
      final RegExpMatch? match = jsonRegex.firstMatch(clean);
  ...
  Map<String, dynamic>? _cleanAndParseJsonMap(String raw) {
    final dynamic result = _cleanAndParseJson(raw);
